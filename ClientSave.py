...
class Client:
	...
	def exitClient(self):  
		...
		if self.state == self.READY or self.state == self.PLAYING or self.state == self.SWITCH:
	...
	def sendRtspRequest(self, requestCode):
		...
		if requestCode == self.SETUP:
			...
			# print('chekc requestsent ' + str(self.requestSent))
		...
		# Teardown request
			...
			if self.state == self.SWITCH:
				if self.filmIndex == 3:
					...
					self.filmIndex += 1
			# print('film index ' + str(self.filmIndex))
			# Keep track of the sent request
		...
		while True: 
			try:	
				...
				if reply:
					...
					# print('check parse')
					# print('reply requestsend ' + str(self.requestSent))
			except:
				break
			...
			if self.sessionId == session:
				if int(lines[0].split(' ')[1]) == 200: 
					if self.requestSent == self.SETUP:
						...
						self.sessionId = 0
						self.requestSent = -1
						# print('reset request sent check')
						# self.rtspSocket.shutdown(socket.SHUT_RDWR)
						# self.rtspSocket.close()
						self.connectToServer()
						self.frameNbr = 0
